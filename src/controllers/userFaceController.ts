...
export class UserFaceController {
    ...
    static async deleteFaceFromS3(imgName: string) {
        try {
            ...
            const fileDelResp = await s3Client.send(command);
            return fileDelResp;
        } catch (error) {
            console.error('Error while delete face from S3:', error);
            throw error;
        }
    }
    ...
    private static async findNearest(embedding: number[], entityId: string): Promise<FaceRecognitionResult | null> {
        try {
            const userFaces = await UserFace.find({ EId: entityId, Del: { $ne: true } })
